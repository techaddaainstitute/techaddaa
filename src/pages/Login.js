...
const Login = () => {
  const { state, sendOtp, verifyOtp, registerProfile, logout } = useStudentAuth();
  ...
  }, [state.user, state.loading, navigate, from]);
  ...
  // Watch for OTP sent state change and transition to step 2
  ...
  return (<BlocConsumer state={state}
    listener={(s) => {
      if (s.sendOtpStatus == Status.SUCCESS) { setStep(2); }
      if (s.verifyOtpStatus == Status.SUCCESS) {
        if (s.isNewUser) {
          setStep(3)
        } else {
          navigate(from, { replace: true });
      ...
      if (s.loginStatus == Status.SUCCESS) {
        navigate(from, { replace: true });
      }
    }}
    builder={(s) => (
                        ...
                        step === 1 ? (
                          ...
                          >
                            <Form onSubmit={async (e) => {
                              ...
                              {state.sendOtpStatus === Status.LOADING ? (
                                <Loading />
                              ) :
                                <Button
                                  type="submit"
                          ...
                          >
                            ...
                            <Form onSubmit={async (e) => {
                              ...
                              {state.verifyOtpStatus === Status.LOADING ? (
                                <Loading />
                              ) :
                                <Button
                                  type="submit"
