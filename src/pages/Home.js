...
const Home = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Fetch all courses
        const allCoursesResult = await CourseUsecase.getAllCoursesUsecase();
        if (allCoursesResult.success) {
          // Convert to mockCourse format for compatibility
          const coursesInMockFormat = CourseUsecase.convertToMockCourseFormat(allCoursesResult.data);
          setCourses(coursesInMockFormat);
        }
  ...
  // Extended courses data for statistics - use real courses if available, fallback to mock
                ...
                <Row className={window.innerWidth < 768 ? "g-3 justify-content-center" : "g-3 g-lg-4"}>
                  ...
                  <Col md={3} sm={6} xs={6} className={window.innerWidth < 768 ? "mb-3" : ""}>
                    ...
                    >
                      ...
                      >
                        {allCourses.reduce((total, course) => total + course.students, 0).toLocaleString()}+
                      </motion.h3>
                  ...
                  <Col md={3} sm={6} xs={6}>
                    ...
                    >
                      ...
                      >
                        {(allCourses.reduce((total, course) => total + course.rating, 0) / allCourses.length).toFixed(1)}
                      </motion.h3>
