-- Migration: Create fees table
-- This table will store fee records for course enrollments
...
CREATE TABLE IF NOT EXISTS public.fees (
    ...
    enrollment_id UUID REFERENCES public.course_enrollments(id), -- Reference to enrollment record if exists
...
-- Policy: Users can insert their own fees (for enrollment)
CREATE POLICY "Users can insert own fees" ON public.fees
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Policy: Users can update their own fees (for payment status)
